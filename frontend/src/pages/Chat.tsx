...
export default function Chat() {
  ...
  // Auto-send initial message on first load
  const hasInitialized = useRef(false);
  useEffect(() => {
    if (!hasInitialized.current && chatId && messages.length === 0 && !isSending) {
      hasInitialized.current = true;
      // Trigger send with empty message to kick off conversation
      setDraft('');
      setIsSending(true);
      setError(null);
      ...
      (async () => {
        try {
          const response = await fetch(CHAT_WEBHOOK_URL, {
            ...
            body: JSON.stringify(buildPayload('', nextMessages, true))
          });
                ...
                for (const line of lines) {
                  ...
                  if (dataStr && dataStr !== '[DONE]') {
                    try {
                      ...
                      hasContent = true;
                      appendAssistantContent(assistantId, dataStr);
                    }
                  }
                }
              ...
              while (newlineIndex !== -1) {
                ...
                if (line) {
                  try {
                    ...
                    hasContent = true;
                    appendAssistantContent(assistantId, line);
                  }
                }
          ...
          if (buffer.trim()) {
            hasContent = true;
            appendAssistantContent(assistantId, buffer);
          }

          if (!hasContent) {
            appendAssistantContent(assistantId, 'Ready to help with your interview review.');
          }
        } catch (err) {
